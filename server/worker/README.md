...
- `shims/` — bundle-time replacements: `express` (recording `Router` with `param`, body-parser factories), `multer` (parses multipart from the Worker `Request`), `jsdom`/`dompurify` (throw → 501; SVG sanitizing only), `pino-http`, `paperclip-db` (package aliases); `adapter-plugin-store` (no external adapters, none disabled), `adapter-plugin-loader` (`buildExternalAdapters` → `[]`), `skills-catalog` (manifest imported from `packages/skills-catalog/generated/catalog.json`, skill files packed by `build.mjs` into `dist/skills-catalog-files.json`); `services-index` (curated barrel, generated); `instrumentation`, `version`, `build-commit`, `build-version` (hand-written, benign values).
...
- `unavailable.ts` — the same two-step lazy proxy the generated stubs use, for values `index.ts` must hand to an unchanged route factory (the plugin loader).
- `live-events.ts` — `LiveEventsRoom` Durable Object (one per company, WebSocket Hibernation) plus the upgrade authorization ported from `src/realtime/live-events-ws.ts`; `shims/live-events.ts` replaces `services/live-events.ts` and forwards each published event to the room through a request-scoped sink.
...
### Mounted today (32 route modules) — the UI runs from the Worker

`dashboard`, `sidebar-badges`, `user-profiles`, `folders`, `goals`, `inbox-dismissals`, `inbox-agent-policy`, `sidebar-preferences`, `resource-memberships`, `decision-training`, `issue-tree-control`, `activity`, `instance-settings`, `costs`, `attention`, `decisions`, `companies` (at `/api/companies`), `access`, `projects`, `pipelines`, `issues`, `approvals`, `routines`, `status-cards`, `agents`, `health` (at `/api/health`), `adapters`, `assets`, `cases`, `company-skills`, `company-skill-policy`, `plugins`, plus `auth` at `/api/auth` — in the order `app.ts` mounts them, because patterns overlap between routers (`access.ts` has `GET /skills/:skillName`, `company-skills.ts` has `GET /skills/catalog`). Byte-identical to Node on 79 of 82 compared GET requests; the 3 that differ answer 501 because they call `heartbeatService` (heartbeat-runs issues, instance task-drain) or the adapter registry (adapter model list). Mutations through the Worker (create/edit/delete goals, projects, issues) are visible from Node and identical on read-back. With the UI build served through Workers Assets, `wrangler dev` alone serves the React app: the dashboard shell loads and its ~30 API calls are answered by the Worker; what it still cannot get is heartbeat runs (501) and environments (404, execution plane). The adapter registry (`src/adapters/registry.ts`) runs on the Worker: the adapter packages follow the per-file rules, so their server entries assemble the `ServerAdapterModule` objects (type, models, capabilities, config schema) while `execute`/login/skills files are lazy stubs — `GET /api/adapters`, `/adapters/:type`, config schemas and model lists match Node (the `opencode_local` list differs only because Node discovers models through the `opencode` CLI installed on that machine). Company skills (`services/company-skills.ts`, allowed: its `node:fs` use is confined to import/scan/sync paths), the skill policy, and the skills catalog (bundled manifest + files) match Node; plugin registry reads (`/plugins`, `/plugins/ui-contributions`, detail) come from the database, while install/worker/job/tool paths answer 501. The live-events WebSocket is served by the `LiveEventsRoom` Durable Object: `GET /api/companies/:id/events/ws` upgrades after the same authorization as Node, and events published by the unchanged services (`activity-log.ts` → `publishLiveEvent`) reach every socket of that company with the same JSON and monotonic ids. `services/issues.ts`, `companies.ts`, `agents.ts`, `approvals.ts`, `routines.ts` run for real; `heartbeat`, `status-cards`, `secrets`, `tool-gateway`, `execution-workspaces`, the native runtime, and the disk catalogs stay stubbed. Storage is R2 (`storage-r2.ts`): image/logo/attachment uploads, downloads (`stream.pipe(res)` through the response shim), and the 10 MB limit answer the same as Node; SVG uploads answer 501 because their sanitizer is `jsdom`.
...
| Live events WebSocket | in-process `EventEmitter` + `ws` | **Done.** `LiveEventsRoom` Durable Object per company + WebSocket Hibernation; publish goes room-ward via a request-scoped sink |
...
| Adapter registry | in-process, adapter packages + external installs from disk | **Done** (built-ins; no external adapter installs) |
| Skills catalog | `@paperclipai/skills-catalog` resolved on disk | **Done.** Manifest + files bundled at build time |
