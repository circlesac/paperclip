...
function definitionDigest(id: string): string {
  return createHash("sha256")
    .update(JSON.stringify(discordPaperclipCommandDefinition(id)))
    .digest("hex");
}
...
function exactDefinition(command: RemoteCommand, id: string): boolean {
  ...
  return (
    ...
    JSON.stringify(discordPaperclipCommandDefinition(id))
  );
}
...
export async function reconcileDiscordCommandRegistration(
  ...
  let state = parseDiscordCommandRegistration(input.state, input.scope);
