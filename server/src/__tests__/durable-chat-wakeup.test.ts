...
describe("durable inbound chat scheduler receipts", () => {
  ...
  it.each([
    ...
    "fences generic recovery by actual original chat ownership ($sourceKind, separate deferred input=$withDeferredInput)",
    async ({ withDeferredInput, sourceKind }) => {
      const f = await retryFixture();
      await db
        .update(agents)
        .set({ adapterType: "durable_chat_retry_test" })
      ...
      const genericRecoveryExpected = sourceKind !== "inbound_wakeup";
      ...
      execute.mockRejectedValueOnce(new Error("Original chat provider failed"));
      ...
      expect(execute).toHaveBeenCalledTimes(
        withDeferredInput || genericRecoveryExpected ? 2 : 1,
      );
      const runs = await db
        .select()
        .from(heartbeatRuns)
      ...
      expect(runs).toHaveLength(
        withDeferredInput || genericRecoveryExpected ? 4 : 3,
      );
      expect(
        runs.some(
          (entry) =>
      ...
      expect(issue).toMatchObject({
        status:
          withDeferredInput || genericRecoveryExpected ? "done" : "blocked",
        executionRunId: null,
      });
      if (withDeferredInput) {
        const [receipt] = await db
